...
module.exports = {
  ...
  createEggFromTwoParents: async function(req, res) {
    ...
    if (
      (!req.body.firstParent && !req.body.secondParent) ||
      req.body.firstParent === req.body.secondParent
    ) {
    ...
    const dbFirstParent = await db.Pet.findOne(
      { _id: req.body.firstParent, user: loggedInUser },
      ["_id", "dna"]
    ).lean(true);
    const dbSecondParent = await db.Pet.findOne(
      { _id: req.body.secondParent, user: loggedInUser },
      ["_id", "dna"]
    ).lean(true);
    ...
    const results = await Promise.all([
      db.Pet.updateOne(
        { _id: dbFirstParent._id, user: loggedInUser },
        { $set: { lastBred: Date.now() } }
      ),
      db.Pet.updateOne(
        { _id: dbSecondParent._id, user: loggedInUser },
        { $set: { lastBred: Date.now() } }
      ),
    ...
    const updatedUserResult = await db.User.findByIdAndUpdate(
      loggedInUser,
      {
  ...
  update: function(req, res) {
    ...
    if (req.body.startIncubate && req.body.duration) {
      ...
      options.$set["willHatchOn"] = moment(req.body.startIncubate, "x").add(
        req.body.duration,
        "milliseconds"
      );
    }
    ...
    db.Egg.findOneAndUpdate(
      { _id: req.params.eggId, user: loggedInUser },
      options,
